...
class MotionPlanning(Drone):
    ...
    def get_colliders_lnglat(self):
        '''
        :returns lat0, lng0 from the colliders file. which is equal to center of the map
        '''
        with open('colliders.csv', newline='') as f:
            reader = csv.reader(f)
            row1 = next(reader)

            lat = float(row1[0].split()[1])
            lng = float(row1[1].split()[1])

        return lng, lat

    def plot_grid(self, grid, start, goal, north_offset, east_offset):
    ...
    # start: lat = 37.792533, lon = -122.397785
    # goal:  lat = 37.793872, lon = -122.395606
    ...
    def plan_path(self):
        ...
        lng, lat = self.get_colliders_lnglat()
        
        # TODO: set home position to (lon0, lat0, 0)
        self.set_home_position(lng, lat, 0)
