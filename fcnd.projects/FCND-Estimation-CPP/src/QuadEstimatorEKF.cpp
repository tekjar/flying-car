...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  // Using the already provided 'IntegrateBodyRate' function
    Quaternion<float> quadQuat = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, ekfState(6));
    quadQuat.IntegrateBodyRate(gyro, dtIMU);
    float predictedPitch = quadQuat.Pitch();
    float predictedRoll = quadQuat.Roll();
    ekfState(6) = quadQuat.Yaw();
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    // Get accelerations in world frame
    V3F accelerationWorld= attitude.Rotate_BtoI(accel);
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    // update state prediction as per section 7.3.1
    zFromX(0) = ekfState(0);
    zFromX(1) = ekfState(1);
    zFromX(2) = ekfState(2);
    zFromX(3) = ekfState(3);
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    hPrime(0, 6) = 1;
    // z is measurement and zFromX is prediction from state
    zFromX(0) = ekfState(6);
    float diff = z(0) - ekfState(6);
    
    // normalize the predection (we could also normalize the measurement)
    if ( diff > F_PI ) {
        zFromX(0) += 2.f * F_PI;
    } else if (diff < - F_PI) {
        zFromX(0) -= 2.f * F_PI;
